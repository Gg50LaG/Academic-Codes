...
return shiftLeftOnce(shiftLeftOnce(keyChunk));
}
// XOR operation
...
for (int j : pc2) {
roundKey.append(combinedKey.charAt(j - 1));
}
...
permText.append(plaintext.charAt(i - 1));
}
String left = permText.substring(0, 32);
String right = permText.substring(32);
for (int i = 0; i < 16; i++) {
String expandedRight = expand(right);
String xored = xor(expandedRight, roundKeys[i]);
...
System.out.println("Round " + (i + 1) + " Cipher Text: " + left + right);
System.out.println("--------------------------------------------------");
}
String combinedText = right + left;
StringBuilder cipherText = new StringBuilder();
for (int i : finalPermutation) {
cipherText.append(combinedText.charAt(i - 1));
}
...
expanded.append(right.charAt(i - 1));
}
...
public static String substitute(String xored) {
StringBuilder output = new StringBuilder();
for (int i = 0; i < 8; i++) {
String sixBitBlock = xored.substring(i * 6, (i + 1) * 6);
...
public static String permute(String substituted) {
StringBuilder permutedOutput = new StringBuilder();
for (int i : P_BOX) {
permutedOutput.append(substituted.charAt(i - 1));
}
...
String plaintext = "0110001101100001011100100110111101110100011101000110111100100000"; // 64-bit input
generateKeys(key);
